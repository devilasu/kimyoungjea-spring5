...
<!-- 메인컨텐츠영역 -->
    <div id="container">
					...
					<c:forEach begin="0" end="1" var="idx">
	                    ...
	                    <c:choose>
	                    	<c:when test="${fn:containsIgnoreCase(checkImgArray,extName)}">
								<img style="max-width:100%;display:block;" alt="다운로드 이미지" src="/image_preview?save_file_name=${boardVO.save_file_names[idx]}">	
							</c:when>
						</c:choose>						
						</c:if>
					</c:forEach>
		...
		<form method="post" name="hide_form" action="" id="hide_form" >
