...
- 사용자단 메인페이지(대시보드) 작업
- 문서작업(제출용) 예정
- 사용자단 네이버아이디 로그인 처리(외부RestAPI구현)
- 관리자 대시보드에서 회원ID 이미지 업로드 및 보이기 처리예정
- 알고리즘 다이어그램 기반 자바코딩테스트 예정.
- 서비스URL (사이트의 로그인URL) -> 네이버로그인폼으로 진행(스프링시큐리티 로그인 무시)
- 네이버로그인폼에서 인증을 받으면(RestAPI에서 OAuth2.0인증) -> 서비스되는 웹으로 돌아오기 위한 서비스 URL(RequestMapping = 콜백URL):스프링시큐리티 로직을 거쳐야 합니다.
- 콜백에서 enabled, ROLE 을 부여
- login_success는 스프링시큐리티의 인증성공 후 이동할 URL 위치 메소드.
- naver_callback은 네이버OAuth2.0 인증성공 후 이동항 URL위치를 구현한 메서드.
